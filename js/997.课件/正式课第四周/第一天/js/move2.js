...
(function(){
    ...
    //问题：1）参数的顺序问题 2）参数的默认值问题；
    function move(curEle,target,duration,effect,callback){//{left:1200,top:400}
        clearInterval(curEle.timer);//开启定时器前先关闭没用的定时器
        //运动形式;
        ...
        //开启定时器，通过Linear公式计算最新位置，并设置最新位置；
        curEle.timer=setInterval(function(){
            if(time+10>=duration){//此处建议应该加上10;预计下一步的情况,不加上10也可以就是跨过目标后在执行这个判断(意思就是跨过去再跨回来)
                utils.css(curEle,target);
                clearInterval(curEle.timer);
                if(typeof callback==='function') callback.call(curEle);//等运动结束后，调用回调函数，同时改变this指向；
                return;
            }
            time+=10;
            for(var attr in begin){
                var curPos=tmpEffect(time,begin[attr],change[attr],duration);
                utils.css(curEle,attr,curPos);//求到最新位置，并设置最新位置；或者把这一行的放到for循环外面去掉attr,,把curPos变为一个对像
            }
        },10)
    }
    window.animate=move;
